...
export default function DatabaseQueryConfig({ node }: ConfigProps) {
  ...
  return (
    <div className="grid gap-3">
      <span className="text-slate-400 text-sm">SQL Query</span>
      <CodeEditor
        value={(node.config.query as string) ?? "SELECT * FROM ..."}
