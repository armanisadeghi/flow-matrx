...
async def list_workflows_endpoint(db: asyncpg.Connection = Depends(get_db)):
    return await list_workflows(db)


@router.post("/", response_model=WorkflowResponse, status_code=status.HTTP_201_CREATED)
async def create_workflow_endpoint(
    payload: WorkflowCreate, db: asyncpg.Connection = Depends(get_db)
):
...
async def update_workflow_endpoint(
    workflow_id: UUID, payload: WorkflowCreate, db: asyncpg.Connection = Depends(get_db)
):
