...
async def get_run_endpoint(run_id: UUID, db: MatrxORM = Depends(get_db)):
...
async def cancel_run_endpoint(run_id: UUID, db: MatrxORM = Depends(get_db)):
    return {"message": "Cancellation requested", "run_id": str(run_id)}


@router.post("/{run_id}/resume", status_code=status.HTTP_202_ACCEPTED)
async def resume_run_endpoint(run_id: UUID, db: MatrxORM = Depends(get_db)):
